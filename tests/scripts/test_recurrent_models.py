...
def test_rnn_many_to_one():
    ...
    for _ in range(15):
        ...
        comp_model = mithril.compile(
            model=ctx, backend=TorchBackend(), constant_keys=input_dict
        )
...
def test_torch_encoder_decoder_var_seq_len():
    ...
    # compile the model
    comp_model = mithril.compile(
        model=ctx, backend=backend, constant_keys=static_inputs
    )
...
def test_lstm_many_to_one():
    ...
    for _ in range(15):
        ...
        constant_keys["initial_hidden"] = torch.swapaxes(h0, 0, 1)
        constant_keys["initial_cell"] = torch.swapaxes(c0, 0, 1)

        # compile the given model
        pm_lstm = mithril.compile(
            model=ctx, backend=backend, constant_keys=constant_keys, jit=True
        )
